...
<body>
    ...
    <div class="ml-72 mx-8 mt-24 mb-24">
        ...
        <div class="flex flex-col gap-4 mt-4">
            <h1 class="font-bold text-3xl">Edit Tabel BK Berdasarkan CPL Program Studi (CPL)</h1>
            <p>
        ...
        <div class="flex flex-col">
            ...
            <form action="{{ route('bk-cpl-mapping.update') }}" name="editCPLPL" method="post">
                @csrf
                @method('PUT')
                <table border="1" cellpadding="5" class="mb-2 text-center w-full mt-4"> 
                    ...
                    <tr class="bg-gray-100">
                        <td class="border-2">Pilihan Kode BK</td>
                    </tr>
                    @foreach ($cpl as $c)
                        <tr>
                            ...
                            <select class="select2 w-full" multiple="multiple" style="width:100%" name="bk_mappings[{{ $c->id_cpl }}][]">
                                    @foreach ($bahan_kajian as $item)
                                        <option value="{{ $item->id_bk }}" 
                                            @if (isset($bk_cpl_map[$c->id_cpl]) && in_array($item->id_bk, $bk_cpl_map[$c->id_cpl])) 
                                                selected 
                                            @endif>
                                            {{ $item->id_bk }} - {{ $item->nama_bk }}
                                        </option>
                                    @endforeach
                                </select>
                            </td>
