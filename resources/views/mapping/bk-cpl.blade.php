...
<body class="bg-gray-100 font-sans">
    ...
    <div class="p-4 sm:p-8 sm:ml-64">
        <main class="mt-20 max-w-4xl mx-auto">
            <form action="{{ route('bk-cpl-mapping.update') }}" method="POST">
                @csrf
                @method('PUT')

                <div class="bg-white p-8 sm:p-10 rounded-xl shadow-lg">
                    <div class="mb-5">
                        <h1 class="text-4xl font-bold text-gray-800">Edit Korelasi BK dan CPL</h1>
                        <p class="text-gray-500 mt-2 text-base">Pilih Bahan Kajian (BK) yang relevan untuk setiap
                                    ...
                                    <tr class="bg-white border-t border-gray-400">
                                        ...
                                        <td class="px-6 py-4">
                                            ...
                                            <select class="select2 w-full" multiple="multiple"
                                                name="bk_mappings[{{ $c->id_cpl }}][]">
                                                @foreach ($bahan_kajian as $item)
                                                    <option value="{{ $item->id_bk }}"
                                                        @if (isset($bk_cpl_map[$c->id_cpl]) && in_array($item->id_bk, $bk_cpl_map[$c->id_cpl])) selected @endif>
