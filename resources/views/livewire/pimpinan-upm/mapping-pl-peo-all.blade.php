<div>
    <div class="mb-6 grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div>
            ...
            <select wire:model.live="selectedKurikulum" id="kurikulum" @if(empty($list_kurikulum)) disabled class="cursor-not-allowed bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5" @endif class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5" >
                    <option value="">
    ...
    <div class="overflow-x-auto max-w-full relative shadow-md sm:rounded-lg border border-gray-400">
        <table class="w-full text-sm text-center border-collapse">
            <thead class="text-xs text-white bg-teks-biru-custom">
                ...
                <tr>
                    @forelse ($peo as $p)
                        <th scope="col" class="px-6 py-3 border border-gray-500 whitespace-nowrap min-w-[100px]" title="{{ $p->desc_peo_id }}">
                            {{ $p->kode_peo }}
                        </th>
                    @empty
                ...
                @if($profil_lulusan)
                    @forelse ($profil_lulusan as $pl)
                        <tr wire:loading.remove wire:target="selectedProdi, selectedKurikulum" class="bg-white border-b border-gray-300 hover:bg-gray-50">
                            ...
                            @forelse ($peo as $p)
                                @php
                                    $isChecked = isset($pl_peo_map[$pl->id_pl]) && in_array($p->id_peo, $pl_peo_map[$pl->id_pl]);
                                @endphp
                    ...
                    @empty
                        <tr wire:loading.remove wire:target="selectedProdi, selectedKurikulum" class="bg-white border-b border-gray-300">
                            <td colspan="2" class="px-6 py-4 text-center text-gray-500">
                                Data Profil Lulusan tidak ditemukan untuk filter ini.
                            </td>
                        </tr>
                    @endforelse
                @else
    ...
    @if($profil_lulusan && $profil_lulusan->hasPages())
        <div >
