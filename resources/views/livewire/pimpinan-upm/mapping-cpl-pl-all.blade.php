<div>
    <div class="mb-6 grid grid-cols-1 md:grid-cols-2 gap-4">
        <div>
            ...
            <select wire:model.live="selectedProdi" id="prodi" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5">
        ...
        <div>
            <label for="kurikulum" class="block mb-2 text-sm font-medium text-gray-900">Pilih Kurikulum</label>
            <select wire:model.live="selectedKurikulum" id="kurikulum" @if(empty($list_kurikulum)) disabled class="cursor-not-allowed bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5" @endif class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5" {{ empty($selectedProdi) ? 'disabled' : '' }}>
                    <option value="">
        ...
        <table class="w-full text-sm text-center border-collapse">
            <thead class="text-xs text-white bg-teks-biru-custom">
                <tr>
                    ...
                    <th scope="col" colspan="{{ count($profil_lulusan) > 0 ? count($profil_lulusan) : 1 }}" class="px-6 py-3 uppercase border border-gray-500 whitespace-nowrap">
                        Profil Lulusan (PL)
                    </th>
                </tr>
                <tr>
                    @forelse ($profil_lulusan as $pl)
                        <th scope="col" class="px-6 py-3 border border-gray-500 whitespace-nowrap min-w-[100px]" title="{{ $pl->desc_pl_id }}">
                            {{ $pl->kode_pl }}
                        </th>
                    @empty
                        <th class="px-6 py-3 italic font-normal border border-gray-500">
                           -
                        </th>
                    @endforelse
                </tr>
            ...
            <tbody>
                ...
                <tr wire:loading wire:target="selectedProdi, selectedKurikulum" class="bg-white border-t border-gray-400 w-full">
                     <td colspan="{{ count($profil_lulusan) + 1 }}" class="px-6 py-4 text-center">
                ...
                @if($cpl)
                    @forelse ($cpl as $cp)
                        <tr wire:loading.remove wire:target="selectedProdi, selectedKurikulum" class="bg-white border-b border-gray-300 hover:bg-gray-50">
                            ...
                            @forelse ($profil_lulusan as $pl)
                                @php
                                    $isChecked = isset($cpl_pl_map[$cp->id_cpl]) && in_array($pl->id_pl, $cpl_pl_map[$cp->id_cpl]);
                                @endphp
                            ...
                            @empty
                                <td class="px-6 py-4 text-gray-400 italic">No PL Data</td>
                            @endforelse
                        </tr>
                    @empty
                        <tr wire:loading.remove wire:target="selectedProdi, selectedKurikulum" class="bg-white border-b border-gray-300">
                            <td colspan="2" class="px-6 py-4 text-center text-gray-500">
                                ...
                                @else
                                    Data CPL tidak ditemukan untuk filter ini.
                                @endif
                            </td>
                        </tr>
                    @endforelse
