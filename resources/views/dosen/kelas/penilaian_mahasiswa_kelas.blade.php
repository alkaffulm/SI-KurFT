                ...
                <div class="bg-white p-8 sm:p-10 rounded-xl shadow-lg">
                    ...
                    <div class="mb-5 mt-12 ">
                        ...
                        <div class="flex justify-between items-center mb-4">
                            <h2 class="text-xl font-bold text-biru-custom">List Daftar Mahasiswa untuk Penilaian Kelas</h2>
                        </div>
                    </div>
                    ...
                    <table class="w-full text-sm text-left text-gray-500">
                        <thead class="text-xs text-white uppercase bg-teks-biru-custom">
                            <tr>
                                ...
                                {{-- loop CPMK (kolom besar) --}}
                                @foreach($cpmk as $c)
                                    @php
                                        // ambil asesmen yg punya bobot untuk cpmk ini
                        ...
                        <tbody>
                            @foreach($kelas->mahasiswa as $mhs)
                                <tr class="text-black">
                                    ...
                                    {{-- loop penilaian mahasiswa (sementara placeholder kosong) --}}
                                    @foreach($cpmk as $c)
                                        @php
                                            $asesmenForCpmk = $bobot->where('id_cpmk', $c->id_cpmk);
                                        @endphp
                                        @foreach($asesmenForCpmk as $ab)
                                            <td class="text-center px-6 py-4">
                                                {{-- nanti diisi nilai mahasiswa, sekarang placeholder --}}
                                                <input type="number" name="nilai[{{ $mhs->id }}][{{ $ab->id_rencana_asesmen }}][{{ $c->id_cpmk }}]" 
