...
<head>
    ...
    <title>Edit Metode Pembelajaran</title>
...
<body class="bg-gray-100 font-sans">
    ...
    <div class="p-4 sm:p-8 sm:ml-64">
        <main class="mt-20 mb-5 max-w-5xl mx-auto">
            ...
            <form id="form-update" action="{{ route('metode-pembelajaran.updateAll') }}" method="POST">
                @csrf
                @method('PUT')

                <div class="bg-white p-8 sm:p-10 rounded-xl shadow-lg">
                    <div class="mb-10">
                        <h1 class="text-4xl font-bold text-gray-800">Edit Metode Pembelajaran</h1>
                    </div>
                    ...
                    <div class="space-y-10">
                        @forelse ($metode_pembelajaran as $tp)
                            <div x-data="{ isDeleting: false }" class="transition-opacity"
                                :class="{ 'is-deleting': isDeleting }">
                                <div class="grid grid-cols-12 gap-x-6 gap-y-4">
                                    <div class="col-span-12 sm:col-span-5">
                                        <label for="nama_metode_pembelajaran_{{ $tp->id_metode_pembelajaran }}"
                                            class="block text-base font-medium text-gray-700 mb-2">Nama Metode Pembelajaran</label>
                                        <input type="text" id="nama_metode_pembelajaran_{{ $tp->id_metode_pembelajaran }}"
                                            name="metode_pembelajaran[{{ $tp->id_metode_pembelajaran }}][nama_metode_pembelajaran]" :disabled="isDeleting"
                                    ...
                                    <div class="col-span-1 flex items-center justify-center pt-8">
                                        ...
                                        <label for="delete_{{ $tp->id_metode_pembelajaran }}"
                        ...
                        @empty
                            <div class="text-center py-16 border-2 border-dashed border-gray-200 rounded-lg">
                                <p class="text-base text-gray-500">Data Metode Pembelajaran belum tersedia untuk diedit.</p>
                            </div>
                        @endforelse
                    </div>

                    @if ($metode_pembelajaran->isNotEmpty())
    ...
    <script>
        document.getElementById('form-update').addEventListener('submit', function(e) {
            ...
            const deleteCheckboxes = document.querySelectorAll('input[name="delete_metode_pembelajaran[]"]:checked');
