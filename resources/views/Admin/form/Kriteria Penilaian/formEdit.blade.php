...
<body class="bg-gray-100 font-sans">
    ...
    <div class="p-4 sm:p-8 sm:ml-64">
        <main class="mt-20 mb-5 max-w-5xl mx-auto">
            ...
            <form action="{{ route('kriteria-penilaian.updateAll') }}" method="POST">
                @csrf
                @method('PUT')

                <div class="bg-white p-8 sm:p-10 rounded-xl shadow-lg">
                    <div class="mb-10">
                        <h1 class="text-4xl font-bold text-gray-800">Edit kriteria Penilaian</h1>
                    </div>
                    ...
                    <div class="space-y-10">
                        @forelse ($kriteria_penilaian as $tp)
                            <div x-data="{ isDeleting: false }" class="transition-opacity"
                                ...
                                <div class="grid grid-cols-12 gap-x-6 gap-y-4">
                                    ...
                                    <div class="col-span-1 flex items-center justify-center pt-8">
                                        <input type="checkbox" id="delete_{{ $cpl->id_cpl }}" name="delete_kriteria_penilaian[]"
...
<body class="bg-gray-100 font-sans">
    ...
    <div class="p-4 sm:p-8 sm:ml-64">
        <main class="mt-20 mb-5 max-w-5xl mx-auto">
            ...
            <form id="form-update" action="{{ route('kriteria-penilaian.updateAll') }}" method="POST">
                @csrf
                @method('PUT')
                ...
                <div class="bg-white p-8 sm:p-10 rounded-xl shadow-lg">
                    ...
                    <div class="space-y-10">
                            ...
                            <div x-data="{ isDeleting: false }" class="transition-opacity"
                                ...
                                <div class="grid grid-cols-12 gap-x-6 gap-y-4">
                                    <div class="col-span-12 sm:col-span-5">
                                        <label for="nama_kriteria_penilaian_{{ $tp->id_kriteria_penilaian }}"
                                            class="block text-base font-medium text-gray-700 mb-2">Nama kriteria Penilaian</label>
                                        <input type="text" id="nama_kriteria_penilaian_{{ $tp->id_kriteria_penilaian }}"
                                            name="kriteria_penilaian[{{ $tp->id_kriteria_penilaian }}][nama_kriteria_penilaian]" :disabled="isDeleting"
                                            class="w-full bg-gray-50 border border-gray-300 text-gray-900 text-base rounded-lg block p-3 transition"
                                            value="{{ old('kriteria_penilaian.' . $tp->id_kriteria_penilaian . '.nama_kriteria_penilaian', $tp->nama_kriteria_penilaian) }}"
                                            required>
                                    </div>
                                    <div class="col-span-1 flex items-center justify-center pt-8">
                                        <input type="checkbox" id="delete_{{ $tp->id_kriteria_penilaian }}" name="delete_kriteria_penilaian[]"
                                            value="{{ $tp->id_kriteria_penilaian }}" class="hidden" x-model="isDeleting">
                                        <label for="delete_{{ $tp->id_kriteria_penilaian }}"
                                            class="cursor-pointer text-gray-400 hover:text-red-600 transition"
                                            title="Tandai untuk dihapus">
                                            <svg :class="{ '!text-red-600': isDeleting }" class="w-7 h-7" fill="none"
                                                stroke="currentColor" viewBox="0 0 24 24"
                        ...
                        @empty
                            <div class="text-center py-16 border-2 border-dashed border-gray-200 rounded-lg">
                                <p class="text-base text-gray-500">Data Kriteria Penilaian belum tersedia untuk diedit.</p>
                            </div>
                        @endforelse
                    </div>

                    @if ($kriteria_penilaian->isNotEmpty())
                        <div class="mt-12 pt-8 border-t border-gray-200 flex justify-end items-center gap-x-4">
    ...
    <script>
        document.getElementById('form-update').addEventListener('submit', function(e) {
            ...
            const deleteCheckboxes = document.querySelectorAll('input[name="delete_kriteria_penilaian[]"]:checked');
