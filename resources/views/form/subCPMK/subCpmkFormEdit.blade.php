...
<body class="bg-gray-100 font-sans">
    ...
    <div class="p-4 sm:p-8 sm:ml-64">
        <main class="mt-20 mb-5 max-w-5xl mx-auto">
            <form id="form-update" action="{{ route('sub-cpmk.updateAll') }}" method="POST" >
                @csrf
                @method('PUT')

                <div class="bg-white p-8 sm:p-10 rounded-xl shadow-lg">
                    <div class="mb-10">
                        <h1 class="text-4xl font-bold text-gray-800">Edit Sub CPMK</h1>
                        <p class="text-gray-500 mt-2 text-base">Ubah detail Sub Capaian Pembelajaran Mata Kuliah (Sub
                            CPMK) di bawah ini.</p>
                    </div>
                    ...
                    <div class="space-y-10">
                        @forelse ($sub_cpmk as $scp)
                            <div x-data="{ isDeleting: false }" class="transition-opacity"
                                ...
                                <div class="grid grid-cols-12 gap-x-6 gap-y-4">
                                    ...
                                    {{-- Baris 1: Kode Sub CPMK dan CPMK Induk --}}
                                    <div class="col-span-12 sm:col-span-6">
                                        <label for="nama_kode_sub_cpmk_{{ $scp->id_sub_cpmk }}"
                                            class="block text-base font-medium text-gray-700 mb-2">Kode Sub CPMK</label>
                                        <input type="text" id="nama_kode_sub_cpmk_{{ $scp->id_sub_cpmk }}"
                                            name="subCpmk[{{ $scp->id_sub_cpmk }}][nama_kode_sub_cpmk]"
                                            class="w-full bg-gray-50 border border-gray-300 text-gray-900 text-base rounded-lg block p-3"
                                            value="{{ old('subCpmk.' . $scp->id_sub_cpmk . '.nama_kode_sub_cpmk', $scp->nama_kode_sub_cpmk) }}"
                                            :disabled="isDeleting" required>
                                    </div>

                                    <div class="col-span-12 sm:col-span-5">
                                        <label for="id_cpmk_{{ $scp->id_sub_cpmk }}"
                                            class="block text-base font-medium text-gray-700 mb-2">CPMK Induk</label>
                                        <select name="subCpmk[{{ $scp->id_sub_cpmk }}][id_cpmk]"
                                            ...
                                            @foreach ($cpmk as $c)
                                                <option value="{{ $c->id_cpmk }}"
                                                    ...
                                                    {{ $c->nama_kode_cpmk }}</option>
                                            @endforeach
                                        </select>
                                    </div>

                                    <div class="col-span-1 flex items-end justify-center pb-2">
                                        <input type="checkbox" id="delete_{{ $scp->id_sub_cpmk }}" name="delete_ids[]"
                                            value="{{ $scp->id_sub_cpmk }}" class="hidden" x-model="isDeleting">
                                        <label for="delete_{{ $scp->id_sub_cpmk }}"
                        ...
                        @empty
                            <div class="text-center py-16 border-2 border-dashed border-gray-200 rounded-lg">
                                <p class="text-base text-gray-500">Data Sub CPMK belum tersedia untuk diedit.</p>
                            </div>
                        @endforelse
                    </div>

                    @if ($sub_cpmk->isNotEmpty())
