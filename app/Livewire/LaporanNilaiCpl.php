...
class LaporanNilaiCpl extends Component
{
    ...
    // -------------------------------------------------------------------------
    // LOAD LAPORAN CPL
    // OPTIMASI: gabungkan semua query menjadi satu JOIN besar
    //           → hanya 1 round-trip ke database, tidak ada loop query
    // Logika identik: (rata_mk × bobot_persen) / 100 per MK, dijumlah per CPL
    // -------------------------------------------------------------------------
    protected function loadLaporanCPL()
    {
        if (!$this->nim || !$this->id_prodi_user) {
        ...
        // OPTIMASI: satu query JOIN sekaligus ambil rata_mk + bobot_persen per CPL-MK
        // Sebelumnya: 2 query terpisah (rata_mk, lalu cplRows) + loop PHP
        // Sekarang  : 1 query, semua kalkulasi tetap di PHP (logika tidak berubah)
        $rows = DB::select(
            "SELECT
                cpl.id_cpl,
                cpl.nama_kode_cpl   AS kode_cpl,
                ...
                mk.nama_matkul_id   AS nama_mk,
                bmu.bobot_persen,
                AVG(pmc.nilai_rata) AS rata_mk
             FROM bobot_mk_untuk_cpl bmu
             JOIN cpl
                ON bmu.id_cpl = cpl.id_cpl
             JOIN mata_kuliah mk
                ON bmu.id_mk = mk.id_mk
             LEFT JOIN mk_cpmk_cpl_map map_tbl
                ON map_tbl.id_mk = bmu.id_mk
             LEFT JOIN penilaian_mahasiswa_cpmk pmc
                ON pmc.id_cpmk = map_tbl.id_cpmk
               AND pmc.nim = ?
             WHERE cpl.id_ps = ?
             GROUP BY
                cpl.id_cpl,
        ...
        // Agregasi di PHP — logika identik dengan versi sebelumnya
        $grouped = [];

        foreach ($rows as $row) {
            $rata_mk   = (float) ($row->rata_mk ?? 0);
            $kontribusi = ($rata_mk * (float) $row->bobot_persen) / 100;
            ...
            $grouped[$row->id_cpl]['details'][] = [
                'id_mk'           => $row->id_mk,
                'nama_mk'         => $row->nama_mk,
                'rata_mk'         => round($rata_mk, 2),
                'bobot_persen'    => (float) $row->bobot_persen,
                'kontribusi_nilai' => round($kontribusi, 2),
            ];
        }
