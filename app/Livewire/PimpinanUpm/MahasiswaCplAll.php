...
class MahasiswaCplAll extends Component
{
...
// Filter Utama
    ...
    public function mount()
    {
        ...
        $this->tahunAkademikList = TahunAkademik::orderBy('tahun_akademik', 'desc')->get();
    }

    // 1. Saat Prodi Dipilih -> Ambil Daftar Kurikulum
    public function updatedSelectedProdi($value)
    {
        // Reset semua child filter
        $this->reset(['selectedKurikulum', 'kurikulumList', 'angkatan', 'nim', 'daftarMahasiswa', 'laporanPerTahun', 'tahunAkademikId', 'cplReportsAngkatan', 'angkatanList']);
        
        if ($value) {
            // Ambil semua kurikulum milik prodi tersebut
            $this->kurikulumList = KurikulumModel::withoutGlobalScopes([ProdiScope::class])
                ->where('id_ps', $value)
                ->orderBy('tahun', 'desc') // Biasanya user cari yang terbaru
                ->get();
        }
    }

    // 2. Saat Kurikulum Dipilih -> Ambil Daftar Angkatan
    public function updatedSelectedKurikulum($value)
    {
        // Reset data di bawahnya
        $this->reset(['angkatan', 'nim', 'daftarMahasiswa', 'laporanPerTahun', 'tahunAkademikId', 'cplReportsAngkatan', 'angkatanList']);

        if ($value && $this->selectedProdi) {
            // Load Angkatan yang ada di Prodi tersebut
            // (Catatan: Angkatan biasanya melekat ke prodi, tapi CPL-nya ikut kurikulum yg dipilih)
    ...
    {
        $this->reset(['angkatan', 'nim', 'daftarMahasiswa', 'laporanPerTahun', 'tahunAkademikId', 'cplReportsAngkatan']);
        // Angkatan list sudah di-load saat pilih kurikulum, jadi aman.
    }

    public function updatedAngkatan($value)
    {
        if (!$this->selectedProdi || !$this->selectedKurikulum) return;
    ...
    {
        // Validasi: Pastikan semua filter utama terisi
        if ($this->mode !== 'mahasiswa' || !$nim || !$this->selectedProdi || !$this->selectedKurikulum) {
            $this->laporanPerTahun = [];
            return;
        }
        ...
        $targetTahunAkademik = collect(range(0, 3))
            ->map(fn ($i) => ($tahunMasuk + $i) . '/' . ($tahunMasuk + $i + 1))
            ->toArray();

        $listTA = TahunAkademik::whereIn('tahun_akademik', $targetTahunAkademik)
        ...
        foreach ($listTA as $ta) {
            ...
            $cpl = $this->hitungKetercapaianCPL($nim, $tahunAkademikIdsAkumulasi);
    ...
    private function loadCplAngkatan()
    {
        ...
        if (!$this->tahunAkademikId || !$this->angkatan || !$this->selectedProdi || !$this->selectedKurikulum) {
            return;
        }
    ...
    // --- Logic Hitung Bobot MK (Tetap Sama) ---
    private function hitungRataRataBobotMK($idMk, $idKelas, $nim)
    {
        ...
        if ($cpmkList->isEmpty()) return null;
        ...
        if ($totalBobotMaksimal == 0) return null;
        return round(($totalBobotTercapai / $totalBobotMaksimal) * 100, 2);
    }

    // --- Logic Hitung CPL (UPDATE: Pakai selectedKurikulum) ---
    private function hitungKetercapaianCPL($nim, $tahunAkademikIds = [])
    {
        // PENTING: Gunakan Kurikulum yang dipilih di dropdown
        $idKurikulum = $this->selectedKurikulum;

        $cplList = DB::table('cpl')
            ->where('id_ps', $this->selectedProdi)
            ->where('id_kurikulum', $idKurikulum) // Filter CPL berdasarkan Kurikulum
            ->select('id_cpl', 'nama_kode_cpl as kode_cpl', 'desc_cpl_id as deskripsi')
            ->get();

        return $cplList->map(function ($cpl) use ($nim, $tahunAkademikIds, $idKurikulum) {
            
            $mappingList = DB::table('mk_cpmk_cpl_map as mccm')
                ->join('mata_kuliah as mk', 'mccm.id_mk', '=', 'mk.id_mk')
                ->leftJoin('mk_cpmk_bobot as mcb', 'mccm.id_mk_cpmk_cpl', '=', 'mcb.id_mk_cpmk_cpl')
                ->where('mccm.id_cpl', $cpl->id_cpl)
                ->where('mk.id_ps', $this->selectedProdi)
                ->where('mk.id_kurikulum', $idKurikulum) // Filter MK sesuai Kurikulum juga
                ->select(
                    'mccm.id_mk_cpmk_cpl',
                    'mccm.id_mk',
                    'mccm.id_cpmk',
            ...
            foreach ($mappingList as $mapping) {
                ...
                foreach ($komponenList as $komponen) {
                    ...
                    if ($penilaian) {
                        $bobotTercapaiMapping += ($penilaian->nilai * $komponen->bobot) / 100;
                    }
                }
    ...
    {
        return view('livewire.pimpinan-upm.mahasiswa-cpl-all', [
            'mode' => $this->mode,
            'prodiList' => $this->prodiList, // Kirim list prodi ke view
            'kurikulumList' => $this->kurikulumList, // Kirim ke View
