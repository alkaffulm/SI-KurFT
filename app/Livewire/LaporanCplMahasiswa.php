...
class LaporanCplMahasiswa extends Component
{
    ...
    private function loadCplAngkatan()
    {
        ...
        foreach ($tahunAkademikList as $ta) {
            ...
            // ---------------------------------------------------------------
            // LOGIKA OBE YANG BENAR:
            // CPL angkatan = Σ bobotTercapai semua mhs / Σ bobotMaksimal semua mhs
            //
            // SALAH (lama): rata-rata nilai_akhir_cpl per mahasiswa
            //   → (CPL_mhs1 + CPL_mhs2 + ...) / n
            //
            // BENAR (baru): akumulasi raw bobot lintas mahasiswa, baru dibagi
            //   → Σ(bobotTercapai_mhs) / Σ(bobotMaksimal_mhs) × 100
            //
            // Perbedaannya: kalau ada mahasiswa yang belum ambil sebagian MK,
            // versi lama akan over-estimate karena pembaginya hanya mahasiswa
            // yang punya nilai, sedangkan versi baru memperhitungkan bobot
            // maksimal seluruh mahasiswa secara proporsional.
            // ---------------------------------------------------------------

            // Struktur: ['kode_cpl' => ['bobot_tercapai' => 0, 'bobot_maks' => 0]]
            $aggregate = [];

            // Inisialisasi aggregate dari daftar CPL agar semua CPL selalu muncul
            ...
            foreach ($nimList as $nim) {
                // hitungRawCplMahasiswa mengembalikan bobot mentah (tercapai & maks)
                // per CPL untuk satu mahasiswa — tidak dinormalisasi ke 0-100 dulu
                $rawRows = $this->hitungRawCplMahasiswa($nim, $tahunAkademikIdsAkumulasi);

                foreach ($rawRows as $kode => $raw) {
                    if (!isset($aggregate[$kode])) {
                        // fallback jika ada CPL di luar cache (seharusnya tidak terjadi)
                        $aggregate[$kode] = [
            ...
            // Normalisasi ke 0-100 setelah semua mahasiswa diakumulasi
    ...
    /**
     * Kembalikan bobot MENTAH (belum dinormalisasi ke 0-100) per CPL
     * untuk satu mahasiswa. Dipakai oleh loadCplAngkatan() agar agregasi
     * lintas mahasiswa dilakukan di level bobot, bukan di level persentase.
     *
     * Return: ['kode_cpl' => ['kode_cpl', 'deskripsi', 'bobot_tercapai', 'bobot_maks']]
     */
    private function hitungRawCplMahasiswa($nim, $tahunAkademikIds = [])
    {
        ...
        foreach ($this->cache['cpl_list'] as $cpl) {
            ...
            foreach ($mappingList as $mapping) {
                ...
                // hitungNilaiCpmkMahasiswa mengembalikan bobot tercapai
                // dalam satuan bobot_cpmk (bukan 0-100), sesuai rumus:
                // (bobotTercapaiAsesmen / totalBobotAsesmen) × bobot_cpmk
    ...
    {
        // Bangun index mapping_by_mk sekali saja (lazy build)
        ...
        foreach ($mappingRows as $idCpmk => $mappingList) {
            foreach ($mappingList as $mapping) {
                ...
                foreach ($asesmenList as $asesmen) {
                    ...
                    // O(1) lookup nilai via cache['nilai_by_key']
